...
import { Service, Rating } from '../lib/supabase';
...
export default function ServiceDetail({ service, onClose }: ServiceDetailProps) {
  ...
  useEffect(() => { loadSellerRatings(); }, [service]);

  async function loadSellerRatings() {
    try {
      const { data, error } = await supabase.from('ratings').select('*').eq('seller_id', service.user_id).order('created_at', { ascending: false });
  ...
  async function handleSubmitRating() {
    ...
    try {
      const { error } = await supabase.from('ratings').insert([{ seller_id: service.user_id, buyer_id: user.id, product_id: service.id, stars: rating, comment }]);
      if (error) throw error;
  ...
  function handleContact() {
    const msg = `Hola, estoy interesado en tu servicio "${service.title}". ¿Puedes ayudarme?`;
    const phone = service.profiles?.phone.replace(/\D/g, '');
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in shadow-2xl">
        <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-gray-100 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-base font-semibold text-gray-900">Detalle del servicio</h2>
        ...
        <div className="p-6">
          <div className="aspect-video bg-gray-50 rounded-2xl mb-6 overflow-hidden">
            {service.image_url ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center">
                <span className="text-6xl font-bold text-blue-200">{service.title.charAt(0)}</span>
              </div>
            )}
          </div>

          <div className="flex items-center gap-2 mb-3">
            <span className="px-2.5 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-full">{service.category}</span>
          ...
          <p className="text-3xl font-bold text-gray-900 mb-4">${service.price.toLocaleString('es-CO')}</p>
          ...
          <div className="border-t border-gray-100 pt-4 mb-6">
            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Prestador</h4>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <img src={service.profiles?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(service.profiles?.full_name || 'U')}&background=16a34a&color=fff&size=128`} alt={service.profiles?.full_name} className="w-10 h-10 rounded-full object-cover" />
                <div>
                  <p className="text-sm font-medium text-gray-900">{service.profiles?.full_name}</p>
                  <p className="text-xs text-gray-400">{service.profiles?.email}</p>
                  <div className="flex flex-wrap gap-1.5 mt-1.5">
                    {service.profiles?.career && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 text-green-700 text-[10px] font-medium rounded-full">{service.profiles.career}</span>
                    )}
                    {service.profiles?.semester && (
                      <span className="inline-flex items-center px-2 py-0.5 bg-blue-50 text-blue-700 text-[10px] font-medium rounded-full">Semestre {service.profiles.semester}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {showContactForm && (
            <div className="bg-gray-50 p-4 rounded-xl mb-4 animate-slide-down">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm text-gray-600">{service.profiles?.phone}</span>
              </div>
          ...
          {showRatingForm && (
            <div className="bg-gray-50 p-4 rounded-xl mb-4 animate-slide-down">
              <h4 className="text-sm font-semibold text-gray-700 mb-3">Calificar prestador</h4>
