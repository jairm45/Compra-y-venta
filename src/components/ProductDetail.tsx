...
import { Product, Rating } from '../lib/supabase';
...
export default function ProductDetail({ product, onClose }: ProductDetailProps) {
  ...
  useEffect(() => { loadSellerRatings(); }, [product]);

  async function loadSellerRatings() {
    try {
      const { data, error } = await supabase.from('ratings').select('*').eq('seller_id', product.user_id).order('created_at', { ascending: false });
      if (error) throw error;
      setSellerRatings(data || []);
      if (data && data.length > 0) {
        setAverageRating(parseFloat((data.reduce((s, r) => s + r.stars, 0) / data.length).toFixed(1)));
      }
  ...
  async function handleSubmitRating() {
    ...
    try {
      const { error } = await supabase.from('ratings').insert([{ seller_id: product.user_id, buyer_id: user.id, product_id: product.id, stars: rating, comment }]);
      if (error) throw error;
  ...
  function handleContact() {
    const msg = `Hola, estoy interesado en tu producto "${product.title}". ¿Está disponible?`;
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in shadow-2xl">
        <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-gray-100 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-base font-semibold text-gray-900">Detalle del producto</h2>
        ...
        <div className="p-6">
          <div className="aspect-video bg-gray-50 rounded-2xl mb-6 overflow-hidden">
            {product.image_url ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-green-50 to-green-100 flex items-center justify-center">
                <span className="text-6xl font-bold text-green-200">{product.title.charAt(0)}</span>
              </div>
            )}
          </div>

          <div className="flex items-center gap-2 mb-3">
            <span className="px-2.5 py-1 bg-green-50 text-green-700 text-xs font-medium rounded-full">{categoryLabels[product.category] || product.category}</span>
          ...
          <p className="text-3xl font-bold text-gray-900 mb-4">${product.price.toLocaleString('es-CO')}</p>
          ...
          <div className="border-t border-gray-100 pt-4 mb-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <img src={product.profiles?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(product.profiles?.full_name || 'U')}&background=16a34a&color=fff&size=128`} alt={product.profiles?.full_name} className="w-10 h-10 rounded-full object-cover" />
                <div>
                  <p className="text-sm font-medium text-gray-900">{product.profiles?.full_name}</p>
                  <p className="text-xs text-gray-400">{product.profiles?.email}</p>
                  <div className="flex flex-wrap gap-1.5 mt-1.5">
                    {product.profiles?.career && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 text-green-700 text-[10px] font-medium rounded-full">{product.profiles.career}</span>
                    )}
                    {product.profiles?.semester && (
                      <span className="inline-flex items-center px-2 py-0.5 bg-blue-50 text-blue-700 text-[10px] font-medium rounded-full">Semestre {product.profiles.semester}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {showContactForm && (
            <div className="bg-gray-50 p-4 rounded-xl mb-4 animate-slide-down">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm text-gray-600">{product.profiles?.phone}</span>
              </div>
